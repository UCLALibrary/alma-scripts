...
class Invoice():
	...
	def _add_line_data(self, inv_line):
		# Special handling of empty note fields - apply default tax code TM
		if inv_line['line_type'] in ['REGULAR', 'OTHER'] and inv_line['note'] is None:
			inv_line['note'] = 'TM'
	...
	def _get_invoice_line(self, xml, ns):
		...
		# Note fields are human entered and thus can be WRONG
	...
	def _get_lbs_tax_code(self, inv_line):
		# LBS enters tax code at start of note, followed by ':'
		# Sometimes they forget the ':', so look for both and take first match.
		if inv_line['note'] is not None:
			return re.split(':| ', inv_line['note'].upper())[0]

	def _get_line_code(self, inv_line):
		# Can depend on different factors: LBS tax code, line type, amount, and note to distinguish shipping / freight from processing charges.
		lbs_tax_code = inv_line['lbs_tax_code']
		note = inv_line['note'].upper()
		...
		if amount < 0:
			...
			line_code = 'ESH'
		elif re.search('SHIPPING CHARGE', note):
			line_code = 'FT'
		elif re.search('EX.*FREIGHT', note):
			line_code = 'FT'
		elif re.search('EX.*PROCESS.*CHARGE', note) or lbs_tax_code == 'EX-PR':
			line_code = 'SVS'
		elif re.search('PROCESS.*CHARGE', note):
			line_code = 'MAT'
